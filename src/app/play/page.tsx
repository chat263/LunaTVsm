...
function PlayPageClient() {
  ...
  const saveCurrentPlayProgress = async (strtype: string = '') => {
    ...
    try {
      if (strtype != 'pause') {
        if (
          currentTime - saveobj.curtime < 10 &&
          saveobj.curtime - currentTime < 10
        ) {
          return;
        }
      }
    ...
    const initPlayer = async () => {
      ...
      artPlayerRef.current.on('error', (err: any) => {
        ...
        artPlayerRef.current.on('video:timeupdate', () => {
          ...
          const interval =
            process.env.NEXT_PUBLIC_STORAGE_TYPE === 'upstash' ? 60000 : 30000; // 统一提高到10秒
          ...
          >
            ...
            <div
              style={{
                ...
                inset: 0,
                background:
                  'linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, rgba(0,0,0,0.45) 60%, rgba(0,0,0,0.7) 100%)',
              }}
            />
          </div>,
          document.body,
